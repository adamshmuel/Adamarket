...
| 15 | 2026-05 | Bottom tab bar labels cut off on iPhone Safari | Three problems compounded: (1) no `<SafeAreaProvider>` at root, so `useSafeAreaInsets()` returned zeros everywhere; (2) the tab bar in `(app)/_layout.tsx` had no bottom padding for the iOS Home Indicator; (3) Expo's default web `index.html` viewport meta tag lacked `viewport-fit=cover`, so iOS Safari never exposed `env(safe-area-inset-*)` to CSS in the first place | Wrapped root in `<SafeAreaProvider>`, padded tab bar by `useSafeAreaInsets().bottom`, patched viewport meta at module load to add `viewport-fit=cover` | `app/_layout.tsx`, `app/(app)/_layout.tsx` |
